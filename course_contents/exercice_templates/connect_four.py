...
def get_user_col_number(grid) -> int:
    result = None
...
def display_player(current_player: int):
    print(f"current player is {current_player}")

def create_row(width: int):
    return [None for _ in range(width)]

def create_grid(width: int, height: int) -> List[List[Optional[int]]]:
    return [create_row(width) for _ in range(height)]

def check_line(lst: List[int], needed: int) -> bool:
    return len(lst) >= needed

def build_consecutive_for_player(line, col_nb, value):
    partial = []
...
def check_if_won(grid, current_player, col_nb, row_nb, needed: int):
    col_nb -= 1
...
def insert_at_column_into_grid(current_player: int, col_nb: int, grid):
    col = col_nb - 1
    height = len(grid)
